...
Bite 102. Infinite loop, input, continue and break
    ...
    Check to see whether they've entered 'quit'. If so, print bye and exit (break) the loop, effectively ending the
    function.
...
def print_colors():
    ...
    while True:
        answer = input("Enter a color or type 'quit' to exit: ")
        if answer == 'quit':
            print("bye")
            break
        elif any(color.lower() == answer.lower() for color in VALID_COLORS):
            print(answer.lower())
        else:
